...
_test('Get access token', assert =>{
  _yelp.accessToken(clientId, clientSecret).then(response => {
    ...
    assert.true(response.access_token);
    assert.equals(response.token_type, 'Bearer');
    assert.true(response.expires_in);
    assert.end();
  }).catch(e =>{
    assert.fail(e);
  });
...
_test('Search', assert =>{
  ...
  client.search({
    ...
    assert.true(response.businesses);
    assert.end();
  }).catch(e =>{
    assert.fail(e);
  });
...
_test('Phone search', assert =>{
  ...
  client.phoneSearch({
    ...
    assert.equals(response.businesses[0].id, 'gary-danko-san-francisco');
    assert.end();
  }).catch(e =>{
    assert.fail(e);
  });
...
_test('Business', assert =>{
  ...
  client.business('gary-danko-san-francisco').then(response => {
    assert.equals(response.name, 'Gary Danko');
    assert.end();
  }).catch(e =>{
    assert.fail(e);
  });
